    ...
    <View style={{gap: theme.spacing.base, flex: 1}}>
      ...
      <View style={{marginTop: 'auto', gap: theme.spacing.sm}}>
        {!speech.isAvailable ? (
          ...
          // than blocking the lesson on a missing native module.
          <>
            <Text variant="caption" center color={theme.colors.textSecondary}>
              Speech check is unavailable on this build.
            </Text>
            <Button label="I said it" onPress={() => onSubmit(true, payload.expectedText)} />
          </>
        ) : graded ? (
          <>
            <Button
              label="Continue"
              variant={graded.isCorrect ? 'primary' : 'danger'}
