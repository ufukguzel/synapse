...
-- A policy filters rows a role can already touch; without this grant every read
-- fails with "42501 permission denied". The same gap shipped once before.
grant select on table public.lesson_vocabulary to authenticated;
