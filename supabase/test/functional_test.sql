...
begin
  ...
  -- Reminder preferences live on notifications_enabled/reminder_time (from
  -- 20260725090400_profile_preferences); the parallel reminder_enabled/
  -- reminder_hour pair from the other line of work was dropped as a duplicate.
  select native_language, timezone, notifications_enabled, reminder_time
  ...
  -- Reminder defaults: off, no time scheduled.
  if v_row.notifications_enabled is not false or v_row.reminder_time is not null then
    raise exception 'FAIL: reminder defaults wrong: enabled=% time=%',
      v_row.notifications_enabled, v_row.reminder_time;
  end if;

  -- Enabling a reminder at a chosen time persists.
  update public.profiles
    set notifications_enabled = true, reminder_time = '08:00'
    where id = v_user;
  select notifications_enabled, reminder_time into v_row
    from public.profiles where id = v_user;
  if v_row.notifications_enabled is not true or v_row.reminder_time <> '08:00'::time then
  ...
  -- Main's expanded seed gives every lesson exactly three exercises.
  select count(*) into v_cnt from public.exercises
    where lesson_id = 'bbbbbbb1-0000-0000-0000-000000000002';
  if v_cnt <> 3 then
    raise exception 'FAIL: lesson 2 should have 3 exercises (seed not idempotent?), got %', v_cnt;
  end if;
